<?php
/**
 * @version	$Id: mtree.php 1378 2012-03-14 01:32:59Z cy $
 * @package	Mosets Tree
	...
	function onContentSearch( $text, $phrase='', $ordering='', $areas=null )
	{
		...
		if( $search_listing && !empty($where) && $limit > 0 ) {
			$sql = 'SELECT DISTINCT l.link_id AS id, l.link_created AS created, l.link_name AS title, cl.cat_id,'
				...
				. " IF(c.cat_id>0, CONCAT_WS('/', '" . $db->getEscaped( JText::_('PLG_SEARCH_MTREE_DIRECTORY') ) . "', c.cat_name), '" . $db->getEscaped( JText::_('PLG_SEARCH_MTREE_DIRECTORY') ) . "') AS section"
		...
		if( $search_category && $limit > 0 )
		{
			$db->setQuery( 'SELECT cat_id AS id, cat_created AS created, cat_name AS title,'
				...
				. " CONCAT_WS('/', '" . $db->getEscaped( JText::_('PLG_SEARCH_MTREE_DIRECTORY') ) . "', cat_name) AS section"
				. ' FROM #__mt_cats '
				. ' WHERE (' . $where_cat . ')'
