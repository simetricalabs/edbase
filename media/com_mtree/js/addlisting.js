jQuery(document).ready(function(){
	...
	jQuery('#mtfields input, #mtfields textarea').on({
...
function onChangeMTFieldsInput(event){
	...
	if(target.type=='checkbox' && target.name.substr(0,5)=='keep_'){
		var file_element=document.mtForm['cf'+field_id];
		if(
			!target.checked
			&&
...
function submitbutton(pressbutton){
	...
	jQuery('#mtfields input[id^="cf"]').each(function(index,el){
		var target=el;
		var validation_failed=false;
	...
	if(jQuery('#'+id).attr('required')=='required'){
		if(target.value==''){
			if(target.type=='file'){
				if(
					(
						(typeof document.mtForm['keep_'+target.name] == 'undefined')
						||
						(typeof document.mtForm['keep_'+target.name] == 'object' && document.mtForm['keep_'+target.name].checked == false)
					)
				){
					return false;
				}else{
...
function mtValidateCheckboxesRadios(target){
	...
	if(
		jQuery('#mtfields input[required][name="'+jQuery('#'+target.id).attr('name')+'"]:checked').length==0
	) {
		return false;
	} else {
		return true;
...
function isEmpty(element){
	var fe=document.mtForm.elements[element];
	if(typeof fe == 'undefined') {
		fe=document.mtForm.elements[element+'[]'];
	}
