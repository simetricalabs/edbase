...
function remSecCat(cat_id){
	...
	jQuery('#mtForm>input[name="other_cats"]').val(new_oc.join(','));
...
function updateMainCat(){
	...
	jQuery('#mtForm>input[name="cat_id"]').val(active_cat);
	toggleMcBut(active_cat);
	togglemc();
	...
	jQuery.getJSON(mosConfig_live_site+"/index.php?option=com_mtree&task=fields.list&cat_id="+active_cat+"&link_id="+linkId+"&format=json"+(jQuery('input[name=is_admin]').val()?'&is_admin=1':''), function(data) {
	  var items = [];
	  jQuery.each(data, function(key, field) {
		...
		if(typeof(cachedFields['field_'+field.id])=='object') {
		    items.push(
				'<li id="field_'+field.id+'" class="'+field.fieldTypeClassName+'">'
		    ...
		    items.push(
				'<li id="field_'+field.id+'" class="'+field.fieldTypeClassName+'">'
				+'<span class="mtcaption" id="caption_'+field.id+'">'
				+'<label for="'+field.name+'">' 
				+ (field.caption?field.caption:'') 
				+(field.isRequired?'<span class="star">&#160;*</span>':'')
				+'</label>'
				+'</span>'
				+'<span id="input_' + field.id + '" class="mtinput">'
	...
	html = items.join('');
	jQuery('#mtfields li[id^="field_"]').remove();
	jQuery(html).appendTo('#mtfields');
	jQuery('#mtfields input').bind('change', function(event) {onChangeMTFieldsInput(event);});
	});
}
function updateCachedFields() {
	var fields = jQuery('#mtfields li[id^=field_]');
...
function cc(parent_cat_id){
	if(parent_cat_id >= 0 && parent_cat_id != '') {
			jQuery.getJSON( mosConfig_live_site+"?option=com_mtree&task=ajax&task2=categories.list&cat_id="+parent_cat_id+"&format=json"+(jQuery('input[name=is_admin]').val()?'&is_admin=1&no_html=1':''), function(data) {
				if(data.length > 0)
				{
					clearList('browsecat');
					var c=0;
					...
					switch(jQuery('#mtForm>input[name="task"]').attr('value')){
						case 'editlink':
						case 'savelisting':
						case 'editcat':
