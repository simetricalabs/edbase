<?php
/**
 * @version	$Id: image.php 1486 2012-06-01 01:35:55Z cy $
 * @package	Mosets Tree
...
class mFieldType_image extends mFieldType_file
{
	...
	function getInputHTML()
	{
		...
		if( $this->attachment > 0 )
		{
			...
			$html .= '<label for="' . $this->getKeepFileName() . '"><img src="' . 
$this->getDataAttachmentURL() . '" hspace="5" vspace="0" /></label>';
		}
		
		$html .= '<input'.($this->isRequired() ? ' required':'').' class="inputbox" type="file" id="' . $this->getInputFieldID() . '" name="' . $this->getInputFieldName(1) . '" />';
		return $html;
	}
