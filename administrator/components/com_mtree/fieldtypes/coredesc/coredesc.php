<?php
/**
 * @version	$Id: coredesc.php 1832 2013-03-14 07:45:48Z cy $
 * @package	Mosets Tree
...
class mFieldType_coredesc extends mFieldType {
	...
	function getInputHTML()
	{
		...
		if( $this->isUsingWysiwygEditor() ) {
			...
			$html = '<textarea'
				...
				. ' class="inputbox'.($this->isRequired() ? ' required':'').'"'
				. ' name="' . $this->getInputFieldName(1).'"'
				. ' id="' . $this->getInputFieldId(1) . '"'
				. ' style="width:100%;height:' . $this->getSize() . 'px">' 
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false ) {
		$html = '';
		$html .= '<input class="inputbox" type="text" name="' . $this->getName();
