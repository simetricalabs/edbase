<?php
/**
 * @version		$Id: upgrade.php 1779 2013-01-18 03:38:06Z cy $
 * @package		Mosets Tree
...
if( !function_exists('runScriptPreflight') )
{
	...
	function preflight($major,$minor,$dev)
	{
		...
		if( $nextUpgradeVersion !== false )
		{
			...
			require($current_dir.DS.'upgrade'.DS.implode('_',$nextUpgradeVersion) . '.php');
	...
	function upgrade($major,$minor,$dev) {
		...
		while($nextUpgradeVersion !== false) {
			...
			{
				require(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade'.DS.implode('_',$nextUpgradeVersion) . '.php');
			}
	...
	function getNextUpgradeVersion($major,$minor,$dev) {
		...
		if(
			file_exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade'.DS. $major . '_' . $minor . '_' .($dev +1) . '.php')
			||
			file_exists($current_dir.DS.'upgrade'.DS. $major . '_' . $minor . '_' .($dev +1) . '.php')
		) {
			return array($major,$minor,($dev +1));
		// Look if there is a next $minor version
		} elseif(
			file_exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade'.DS. $major . '_' . ($minor +1) . '_0.php')
			||
			file_exists($current_dir.DS.'upgrade'.DS. $major . '_' . ($minor +1) . '_0.php')
		) {
			return array($major,($minor +1),0);
		// Look if there is a next $major version
		} elseif(
			file_exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade'.DS. ($major +1) . '_0_0.php')
			||
			file_exists($current_dir.DS.'upgrade'.DS. ($major +1) . '_0_0.php')
		) {
