<?php
/**
 * @version	$Id: customfields.mtree.php 1839 2013-03-21 03:05:14Z cy $
 * @package	Mosets Tree
...
function editcf( $cf_id, $option ) {
	...
	if ($row->cf_id == 0) {
		...
		$fieldtype_path = JPATH_ROOT . $mtconf->get('relative_path_to_fieldtypes') . $row->field_type . DS;
	...
	if($row->field_type == 'coreuser') {
		...
		$lists['filter_search'] = JHTML::_('select.booleanlist', 'filter_search', 'class="inputbox" disabled', 0);
		$lists['simple_search'] = JHTML::_('select.booleanlist', 'simple_search', 'class="inputbox" disabled', 0);
	} else {
		$lists['advanced_search'] = JHTML::_('select.booleanlist', 'advanced_search', 'class="inputbox"', (($row->advanced_search == 1) ? 1 : 0));
		$lists['filter_search'] = JHTML::_('select.booleanlist', 'filter_search', 'class="inputbox"', (($row->filter_search == 1) ? 1 : 0));
	...
	if ($row->cf_id == 0) {
		...
		$database->setQuery($sql);
		$fields_map_cats = $database->loadResultArray();
	}
	
	HTML_mtcustomfields::editcf( $row, $custom_cf_types, $lists, $cats, $fields_map_cats, $form, $option );
}
