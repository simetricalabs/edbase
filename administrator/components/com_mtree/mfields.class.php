<?php
/**
 * @version	$Id: mfields.class.php 1831 2013-03-13 11:48:26Z cy $
 * @package	Mosets Tree
...
class mFields {
	...
	function getFieldTypeClassName( $data=array() ) {
		...
		if(class_exists('mFieldType_' . $data['fieldType'])) {
			...
			$fieldtype_file = JPATH_ROOT . $mtconf->get('relative_path_to_fieldtypes') . $data['fieldType'] . DS  . $data['fieldType'] . '.php';
...
class mFieldType {
	...
	public function __construct( $data=array() )
	{
		if( !is_null($data) )
		{
			...
			$fieldType = substr(get_class($this),11);
			if( !empty($fieldType) )
			{
	...
	function getSearchValue() {
		...
		// Check if cookie contains search value information
			...
			{
				$searchValueCookie = JRequest::getVar( 
					'com_mtree_mfields_searchFieldValue_'.$this->getId(),
					'', 
					'COOKIE'
					);
	...
	function getInputHTML()
	{
		...
		{
			$html = '<select'.($this->isRequired() ? ' required':'').' name="' . $this->getInputFieldName(1) . '" id="' . $this->getInputFieldID(1) . '" class="inputbox text_area">';
		...
		else
		{
			$html = '<input'
				...
				. ' class="inputbox text_area'.($this->isRequired() ? ' required':'')
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		if( !empty($this->arrayFieldElements[0]) )
		{
			...
			$html .= '<input class="inputbox text_area" type="text" name="' . $this->getName();
	...
	function getFieldTypeAttachmentURL($arg) {
		global $mtconf;
		return JURI::root() . $mtconf->get('relative_path_to_fieldtypes_media') . $this->fieldType . DS . $arg;
		// return JURI::root() . $mtconf->get('relative_path_to_fieldtypes_media') . DS . $this->fieldType . DS . $arg;
	}
	...
	*/
	...
	function getWhereCondition() {
		if( func_num_args() == 0 ) {
			...
			if( $this->isCore() ) {
				return $this->getName() . ' LIKE \'%' . JFactory::getDBO()->getEscaped( func_get_arg(0), true ) . '%\'';
			} else {
				return '(cfv#.value LIKE \'%' . JFactory::getDBO()->getEscaped( func_get_arg(0), true ) . '%\')';
			}
		}
	}
	...
	function parseMambots( &$html ) {
		...
		if(defined('JVERSION')) {
			...
			$results = $_MAMBOTS->trigger( 'onContentPrepare', array( 'com_mtree.field', &$link, &$params, $page ), true );
		}
		$html = $link->text;			
...
class mFieldType_weblink extends mFieldType {
	...
	function getInputHTML()
	{
		...
		$html = '';
		$html .= '<input class="text_area inputbox" type="url"'
		...
		if($showGo && $this->inBackEnd())
		{
			...
			$html .= 'value="' . JText::_( 'COM_MTREE_GO' ) . '" />';
		}
		return $html;
	}
	...
	function getJSValidationFunction() {
		return 'function(){return /^(http:\/\/|https:\/\/)?([a-zA-Z0-9]+\.[a-zA-Z0-9\-]+|[a-zA-Z0-9\-]+)\.[a-zA-Z\.]{2,6}(\/[a-zA-Z0-9\.\?=\/#%&\+-]+|\/|)/i.test(arguments[0].value)}';
	}

	function getJSValidationMessage() {
		return JText::_( 'COM_MTREE_PLEASE_ENTER_A_VALID_URL' );
	}
	...
	function getOutput() {
		...
		if( $showCounter )
		{
			$html .= '<span class="counter">('.JText::sprintf('COM_MTREE_NUMBER_OF_VISITS', $this->counter).')</span>';
		}
...
class mFieldType_selectmultiple extends mFieldType
{
	...
	function getWhereCondition() {
		...
		if( is_array($args) ) {
			foreach( $args AS $arg ) {
				$where[] = 'cfv#.value LIKE \'%' . JFactory::getDBO()->getEscaped( $arg, true ) . '%\'';
			}
		}
		if( count($where) > 1 ) {
...
class mFieldType_radiobutton extends mFieldType {
	...
	function getInputHTML()
	{
		...
		foreach($this->arrayFieldElements AS $fieldElement)
		{
			if(!empty($fieldElement)) {
				...
				$html .= '/><label for="' . $this->getInputFieldID(1) . '_' . $i . '">'.$fieldElement.'</label>';
				$html .= '</li>';
				$i++;
			}
		}
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		foreach($this->arrayFieldElements AS $fieldElement) {
			if(!empty($fieldElement)) {
				...
				$html .= '/><label for="' . $idprefix . $this->getName() . '_' . $i . '">'.$fieldElement.'</label>';
				$html .= '</li>';
				$i++;
			}
		}
	...
	function getWhereCondition() {
		if( func_num_args() == 0 ) {
			...
			return '(cfv#.value = \'' . JFactory::getDBO()->getEscaped( func_get_arg(0), true ) . '\')';
		}
	}
}
...
class mFieldType_file extends mFieldType {
	...
	function getOutput() {
		...
		if( $showCounter )
		{
			$html .= '<span class="counter">('.JText::sprintf('COM_MTREE_NUMBER_OF_VIEWS', $this->counter).')</span>';
		}
	...
	function getInputHTML()
	{
		...
		if( $this->attachment > 0 ) {
			...
			$showCounter = $this->getParam('showCounter',1);
			if( $showCounter ) {
				$html .= ' (' . JText::sprintf('COM_MTREE_NUMBER_OF_VIEWS', $this->counter) . ')';
			}
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		$html = '';
		$html .= '<input class="inputbox text_area" type="checkbox"';
		...
		$html .= ' />';
		$html .= '&nbsp;';
		$html .= '<label for="' . $idprefix . $this->getName() . '">';
		$html .= JText::_( 'COM_MTREE_CONTAINS_FILE' );
		$html .= '</label>';
		return $html;
	}
...
class mFieldType_number extends mFieldType {
	...
	function getJSValidationMessage() {
		return JText::_( 'COM_MTREE_PLEASE_ENTER_A_VALID_NUMBER' );
	}
	...
	function validateValue($value)
	{
		if(is_numeric($value)) {
			...
			$this->setError(JText::_( 'COM_MTREE_PLEASE_ENTER_A_VALID_NUMBER' ));
			return false;
		}
	}
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		$operators = array(
			''	=> '',
			'1'	=> JText::_( 'COM_MTREE_EXACTLY' ),
			'2'	=> JText::_( 'COM_MTREE_MORE_THAN' ),
			'3'	=> JText::_( 'COM_MTREE_LESS_THAN' )
		);
...
class mFieldType_date extends mFieldType {
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		$html = '';
		$html .= '<input style="float:none" id="' . $this->getSearchFieldName(1) . 'a"';
		$html .= ' name=' . $this->getSearchFieldName(1);
		$html .= ' type="radio"';
		$html .= ' value="1"';
		...
		$html .= ' />';
		$html .= '<label for="' . $this->getSearchFieldName(1) . 'a">' . JText::_( 'COM_MTREE_EXACTLY_ON' ) . '</label>&nbsp;';
		
		$html .= '<select name="' . $this->getSearchFieldName(2) . '" class="inputbox">';
		$html .= '<option value="">&nbsp;</option>';
		for($day=1;$day<=31;$day++) { 
			$html .= '<option value="' . $day . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(2)] == $day ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= $day;
		...
		for($month=1;$month<=12;$month++) { 
			...
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(3)] == $month ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= strftime('%b', mktime(0, 0, 0, $month));
			$html .= '</option>';
		}
		$html .= '</select>';
		...
		for($year=$endYear;$year>=$startYear;$year--) {
			...
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(4)] == $year ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= $year;
		...
		$html .= ' />';
		$html .= '<label for="' . $this->getSearchFieldName(1) . 'b">' . JText::_( 'COM_MTREE_BETWEEN' ) . '</label>&nbsp;';
		
		$html .= '<select name="' . $this->getSearchFieldName(5) . '" class="inputbox">';
		$html .= '<option value="">&nbsp;</option>';
		for($day=1;$day<=31;$day++) {
			$html .= '<option value="' . $day . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(5)] == $day ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= $day;
		...
		for($month=1;$month<=12;$month++) {
			$html .= '<option value="' . $month . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(6)] == $month ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= strftime('%b', mktime(0, 0, 0, $month, 1));
			$html .= '</option>';
		}
		$html .= '</select>';
		...
		for($year=$endYear;$year>=$startYear;$year--) {
			$html .= '<option value="' . $year . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(7)] == $year ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= $year;
		...
		for($day=1;$day<=31;$day++) {
			$html .= '<option value="' . $day . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(8)] == $day ) {
				$html .= ' selected=selected';
			}
			$html .= '">';
			$html .= $day;
		...
		for($month=1;$month<=12;$month++) {
			$html .= '<option value="' . $month . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(9)] == $month ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= date("M", mktime(0, 0, 0, $month, 1));
			$html .= '</option>';
		}
		$html .= '</select>';
		...
		for($year=$endYear;$year>=$startYear;$year--) {
			$html .= '<option value="' . $year . '"';
			if( $showSearchValue && $searchValue[$this->getSearchFieldName(10)] == $year ) {
				$html .= ' selected=selected';
			}
			$html .= '>';
			$html .= $year;
	...
	function getInputHTML() {
		...
		$html = '';
		$html .= '<select'.($this->isRequired() ? ' required':'').' name="' . $this->getInputFieldName(1) . '" class="inputbox">';
		$html .= '<option value="">&nbsp;</option>';
		for($day=1;$day<=31;$day++) {
			$html .= '<option value="' . $day . '"';
			if( $day == $dayValue )
		...
		$html .= '<select'.($this->isRequired() ? ' required':'').' name="' . $this->getInputFieldName(2) . '" class="inputbox">';
		$html .= '<option value="">&nbsp;</option>';
		for($month=1;$month<=12;$month++) {
			$html .= '<option value="' . $month . '"';
			if( $month == $monthValue )
		...
		$html .= '<select'.($this->isRequired() ? ' required':'').' name="' . $this->getInputFieldName(3) . '" class="inputbox">';
		$html .= '<option value="">&nbsp;</option>';
		for($year=$endYear;$year>=$startYear;$year--) {
			$html .= '<option value="' . $year . '"';
			if( $year == $yearValue )
	...
	function getWhereCondition() {
		...
		if($args[0] == 1) {
			if( is_numeric($args[1]) && is_numeric($args[2]) && is_numeric($args[3]) ) {
				return 'STR_TO_DATE('.$fieldname.',\'%Y-%m-%d\') = STR_TO_DATE(\'' . JFactory::getDBO()->getEscaped( $date0 ) .'\',\'%Y-%m-%d\')';
			}
		} else {
			if( is_numeric($args[4]) && is_numeric($args[5]) && is_numeric($args[6]) && empty($args[7]) && empty($args[8]) && empty($args[9]) ) {
				return 'STR_TO_DATE('.$fieldname.',\'%Y-%m-%d\') >= STR_TO_DATE(\'' . JFactory::getDBO()->getEscaped( $date1 ) .'\',\'%Y-%m-%d\')';
			} elseif( is_numeric($args[7]) && is_numeric($args[8]) && is_numeric($args[9]) && empty($args[4]) && empty($args[5]) && empty($args[6]) ) {
				return 'STR_TO_DATE('.$fieldname.',\'%Y-%m-%d\') <= STR_TO_DATE(\'' . JFactory::getDBO()->getEscaped( $date2 ) .'\',\'%Y-%m-%d\')';
				...
				if($maxDate == $minDate) {
					...
					return '(STR_TO_DATE('.$fieldname.',\'%Y-%m-%d\') >= STR_TO_DATE(\'' . JFactory::getDBO()->getEscaped( $minDate ) .'\',\'%Y-%m-%d\') AND STR_TO_DATE('.$fieldname.',\'%Y-%m-%d\') <= STR_TO_DATE(\'' . JFactory::getDBO()->getEscaped( $maxDate ) .'\',\'%Y-%m-%d\'))';
				}
			} else {
				return null;
			}
...
class mFieldType_email extends mFieldType {
	...
	function getJSValidationMessage() {
		return JText::_( 'COM_MTREE_PLEASE_ENTER_A_VALID_EMAIL' );
	}
	...
	function validateValue( $value )
	{
		...
		{
			$this->setError(JText::_( 'COM_MTREE_PLEASE_ENTER_A_VALID_EMAIL' ));
	        	return false;
		}
		else
		{
	...
	function getInputHTML()
	{
		$html = '';
		$html .= '<input'.($this->isRequired() ? ' required':'');
		$html .= ' class="inputbox text_area'.($this->isRequired() ? ' required':'');
...
class mFieldType_tags extends mFieldType {
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		foreach( $rawTags AS $tag => $items )
		{
			...
			$html .= '/>';
			$html .= '<label for="' . $this->getName() . '_' . $i . '" id="' . $tags[$i]->elementId . '">';
			$html .= $tags[$i]->value;
			$html .= '</label>';
			$html .= '</li>';
			// $html .= '<br>';
	...
	function getInputHTML()
	{
		...
		$html = '';
		$html .= '<input'.($this->isRequired() ? ' required':'');
		$html .= ' class="inputbox text_area" type="text" name="' . $this->getInputFieldName(1) . '"';
		$html .= ' id="' . $this->getInputFieldID(1) . '"';
		$html .= ' size="' . $this->getSize() . '"';
