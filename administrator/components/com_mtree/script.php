<?php
/**
 * @version	$Id: script.php 1801 2013-02-13 11:05:57Z cy $
 * @package	Mosets Tree
...
include_once( 'administrator'.DS.'components'.DS.'com_mtree'.DS.'config.mtree.class.php' );

class Com_MtreeInstallerScript {
	
	function preflight( $type, $parent )
	{
		require_once( dirname(__FILE__).DS.'administrator'.DS.'components'.DS.'com_mtree'.DS.'upgrade.php' );
		runScriptPreflight();
	}
	...
	function install($parent)
	{
		...
		if( !is_null($num_of_cats) && $num_of_cats == 0 )
		{
			...
			$this->loadSampleData(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'sample-mt-data.sql');

			// Run populate_fields_map() from 3_0_0 to populate #__mt_fields_map table.
			require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade.class.php');
			require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.'upgrade'.DS.'3_0_0.php');
	...
	function loadSampleDataScript($sqlfile='sample.sql')
	{
		...
		$sqlfile_path = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mtree'.DS.$sqlfile;
