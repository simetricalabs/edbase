<?php
/**
 * @version	$Id: router.php 1842 2013-03-26 10:45:28Z cy $
 * @package	Mosets Tree
	...
	switch($end_segment) {
		...
		case ($total_segments > 1 && $mtconf->get('sef_searchby') == $segments[$total_segments-2]):
			if( $mtconf->get('sef_searchby') == $segments[$total_segments-2] ) {
				...
				$vars['value'] = JRequest::getString('value', null, 'query');
		...
		case $total_segments > 2 && $mtconf->get('sef_search') == $segments[$total_segments-3]:
		case $total_segments > 1 && $mtconf->get('sef_search') == $segments[$total_segments-2]:
			if( $mtconf->get('sef_search') == $segments[$total_segments-2] ) {
				$page = 1;
				...
				$vars['searchword'] = $segments[$total_segments-2];
			}
		...
		case $total_segments == 1 && $mtconf->get('sef_search') == $segments[$total_segments-1]:
			$vars['searchword'] = '';
			$vars['task'] = 'search';
			$vars['limit'] = $mtconf->get('fe_num_of_searchresults');
			$vars['limitstart'] = 0;
...
function appendCat($cat_id, &$query)
{
	...
	{
		if( isset($menu_item->query['view']) && $menu_item->query['view'] == 'listcats' )
		{
