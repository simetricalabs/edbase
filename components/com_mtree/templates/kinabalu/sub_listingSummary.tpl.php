...
?>" data-link-id="<?php echo $link->link_id; ?>">
		...
		while( $link_fields->hasNext() ) {
			...
			if(
				...
				echo "\n\t\t\t" . '<div id="field_'.$field->getId().'" style="width:'.floor(98/$number_of_columns).'%" class="fieldRow'.(($number_of_columns == 1 || $field_count % $number_of_columns == 0)?' lastFieldRow':'').'">';
		...
		if($this->config->getTemParam('showActionLinksInSummary','0')) {
			...
			$this->plugin( 'ahrefvisit', $link, array("rel"=>"nofollow") );
