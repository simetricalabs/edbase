...
?>
<li class="button-send"><input type="submit" value="<?php echo JText::_( 'COM_MTREE_SEARCH' ) ?>" onclick="javascript:var cookie = document.cookie.split(';');for(var i=0;i < cookie.length;i++) {var c = cookie[i];while (c.charAt(0)==' '){c = c.substring(1,c.length);}var name = c.split('=')[0];if( name.substr(0,35) == 'com_mtree_mfields_searchFieldValue_'){document.cookie = name + '=;';}}" /></li>
<li class="button-reset"><input type="button" value="<?php echo JText::_( 'COM_MTREE_RESET' ) ?>" onclick="javascript:var form=jQuery('form[name=mtFormAllListings] input,form[name=mtFormAllListings] select');form.each(function(index,el){if(el.type=='checkbox'||el.type=='radio'){el.checked=false;}if(el.type=='text'){el.value='';}if(el.type=='select-one'||el.type=='select-multiple'){el.selectedIndex='';}});jQuery('form[name=mtFormAllListings]').submit();var cookie = document.cookie.split(';');for(var i=0;i < cookie.length;i++) {var c = cookie[i];while (c.charAt(0)==' '){c = c.substring(1,c.length);}var name = c.split('=')[0];if( name.substr(0,35) == 'com_mtree_mfields_searchFieldValue_'){document.cookie = name + '=;';}}" /></li>
</ul>
</div>
<?php else: 
