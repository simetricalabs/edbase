<?php
/**
 * @version	$Id: mtree.php 1838 2013-03-20 01:14:43Z cy $
 * @package	Mosets Tree
...
require(  JPATH_COMPONENT.DS.'init.php' );
...
require_once( JPATH_COMPONENT.DS.'methods.php' );
...
switch ($task) {
	...
	case "toplisting":
		...
		$sort	= JRequest::getCmd('sort', $mtconf->get('all_listings_sort_by'));
	...
	/* JSON List Fields */
	case "fields.list":
		require_once JPATH_COMPONENT.DS.'fields.json.php';
		break;
...
if( $mtconf->get('load_css') && $document->getType() == 'html')
{
	if ( file_exists( $savantConf['template_path'] . 'template.css' ) ) {
		if( $mtconf->getjconf('absolute_path') == DS )
		{
		...
		if( $app->getCfg('sef') )
		{
			...
			$document->addHeadLink( 
				$uri->toString(array( 'scheme', 'host', 'port' )) 
					. JRoute::_(
						'index.php?option=com_mtree&task=listcats&cat_id='.$cat_id
...
function searchby( $option )
{
	...
	else
	{
		...
		$database->setQuery('SELECT * FROM #__mt_customfields WHERE published = 1 AND tag_search = 1 ORDER BY ordering ASC');
		...
		jimport('joomla.html.pagination');
		$pageNav = new JPagination(count($taggable_fields), $limitstart, $mtconf->get('fe_num_of_searchbytags'));
	...
	if( empty($value) )
	{
		...
		$arrTags = $database->loadResultArray();
...
function search( $option ) {
	...
	$post['search_cat'] = JRequest::getInt('search_cat', 0);
...
function listalpha_cache( $cat_id, $alpha, $limitstart, $option ) {
	...
	# Alphabets (A-Z)
		...
		$where[] = "link_name LIKE '" . $database->getEscaped( $alpha, true ) . "%' COLLATE utf8_swedish_ci ";
		$where_cat[] = "cat1.cat_name LIKE '" . $database->getEscaped( $alpha, true ) . "%' COLLATE utf8_swedish_ci ";
...
function listlisting( $cat_id, $option, $my, $task, $sort, $limitstart ) {
	...
	# Set title
	// setTitle( JText::sprintf('COM_MTREE_PAGE_TITLE_LIST_LISTINGS', $cat_name, $listListing->getTitle()) );
...
function listlisting_cache( $cat_id, $cat_name, $option, $listListing ) {
	...
	if( isset($post['cfcat_id']) )
	{
		$cfcat_id	= intval( $post['cfcat_id'] );
		$search_params[$post['cfcat']] = $cfcat_id;
	} else {
		$cfcat_id	= 0;
	}
	...
	# Load all CORE and custom fields
	$database->setQuery( "SELECT cf.*, '0' AS link_id, '' AS value, '0' AS attachment, '".$cat_id."' AS cat_id FROM #__mt_customfields AS cf "
		.	"\nWHERE cf.hidden ='0' AND cf.published='1' && (filter_search = '1' || advanced_search = '1')"
		.	((!empty($cf_ids))?"\nAND cf.cf_id IN (" . implode(',',$cf_ids). ") ":'')
		.	" ORDER BY ordering ASC" 
		);
	$fields = new mFields($database->loadObjectList());
	...
	if( !empty($searchParams) || $cfcat_id > 0 )
	{
		...
		if( isset($post['searchcondition']) && intval( $post['searchcondition'] ) == 1 ) {
	...
	// $savant->assign('cat_name', $cat_name);
	$savant->assign('cat', $cat);
...
function getOwnerProfile( $user_id )
{
	...
	$owner_model = JModel::getInstance('User', 'UsersModel', array('ignore_request' => true));
...
function viewlink_cache( $link, $limitstart, $fields, $params, $my, $option ) {
	...
	if ( !isset($link->link_id) || $link->link_id <= 0 ) {
		...
		# Page Navigation
		// jimport('joomla.html.pagination');
		// $reviewNav = new JPagination($total_reviews, $limitstart, $mtconf->get('fe_num_of_reviews'));
		...
		// $savant->assign('reviewNav', $reviewNav);
		$savant->assign('my', $my);
		$savant->assign('reviews', $reviews);
		$savant->assign('images', $images);
		$savant->assign('voted_reviews', $voted_reviews);
		...
		{
			if( jimport('mosets.profilepicture.profilepicture') )
			{
				$savant->assign('profilepicture_url', '');
...
function send_report( $link_id, $option ) {
	...
	if ( $mtconf->get('show_report') == 0 ) {
		...
		$user_fields_data['your_name']		= JRequest::getString('your_name', '', 'post');
		$user_fields_data['report_type']	= JRequest::getInt('report_type', '', 'post');
		$user_fields_data['message']		= JRequest::getVar( 'message',	'', 'post');
		$captcha_answer                 	= JRequest::getString( 'recaptcha_response_field', '', 'post'); 
...
function addreview( $link_id, $option ) {
	...
	# Get the review data
	$user_fields_data['rev_text']	= JRequest::getString( 'rev_text', '', 'post');
	$user_fields_data['rev_title'] 	= JRequest::getString( 'rev_title', '', 'post');
	$user_fields_data['guest_name']	= JRequest::getString( 'guest_name', '', 'post');
	$captcha_answer			= JRequest::getString( 'recaptcha_response_field', '', 'post');
	$remote_addr			= JRequest::getCmd( 'REMOTE_ADDR', '', 'server');
	...
	{
		$user_fields_data['rating'] = JRequest::getInt('rating', 0, 'post');
	...
	// Validate Inputs
	if ( !validateInputs( $user_fields_data['guest_name'], $user_fields_data['rev_text'], $user_fields_data['rev_text'] ) )
	{
		$document =& JFactory::getDocument();
		JError::raiseWarning( 0, $document->getError() );
		return false;
...
function addrating( $link_id, $option ) {
	...
	if( $result ) {
		...
		// echo JText::_( 'COM_MTREE_THANKS_FOR_RATING' ) . '|' . $total_votes . ' ' . strtolower(JText::_( 'COM_MTREE_VOTES' ));
	} else {
		$return->status = 'NA';
		// echo 'NA';
	}
...
function votereview( $rev_id, $rev_vote, $option ) {
	...
	if( $result ) {
		...
		// $return = JText::sprintf( 'COM_MTREE_PEOPLE_FIND_THIS_REVIEW_HELPFUL', (($rev_vote == 1)? $review->vote_helpful +1:$review->vote_helpful), ($review->vote_total +1) );
		// $return .= '|'.JText::_( 'COM_MTREE_THANKS_FOR_YOUR_VOTE' );
		// echo $return;
	} else {
		$return->status 	= 'NA';
		// echo 'NA';
...
function send_reportreview( $rev_id, $option ) {
	...
	if( $mtconf->get('user_report_review') == -1 || ($mtconf->get('user_report_review') == 1 && $my->id == 0) ) {
		...
		if( count($link) == 1 && $link->link_id > 0 )
		{
			...
			{
				$user_fields_data['your_name'] = JRequest::getVar( 'your_name', '', 'post');
			}
...
function replyreview( $rev_id, $option ) {
	...
	if( isset($review) && $review->link_id > 0 && $my->id > 0 && $mtconf->get('owner_reply_review') ) {
		...
		if( $link->user_id == $my->id ) {
			setTitle(JText::_( 'COM_MTREE_PAGE_TITLE_REPLY_REVIEW', $review->rev_title ));
...
function send_contact( $link_id, $option ) {
	...
	if ( 
		...
		$data		= JRequest::getVar('mtform', array(), 'post', 'array');
		$captcha_answer	= JRequest::getString( 'recaptcha_response_field', '', 'post'); 
		$uri		=& JURI::getInstance();
	...
	if ( ($mtconf->get('user_addlisting') == 1 && $my->id < 1) || ($link_id > 0 && $my->id == 0) )
	{
		JFactory::getApplication('site')->redirect(
			JRoute::_(
				'index.php?option=com_users&view=login&return='
				. base64_encode( 
					JRoute::_('index.php?option=com_mtree&task=addlisting&link_id='.$link->link_id) 
				)
			),
			JText::sprintf( 
...
function savelisting( $option ) {
	...
	$other_cats = JRequest::getString('other_cats', null, 'post');
	if( $other_cats == '' ) {
		$other_cats = array();
	} else {
	...
	else
	{
		...
		if(isset($keep_att_ids) && !empty($keep_att_ids) ) {
			...
			$att_ids_to_be_removed_if_validate = array_merge($att_ids_to_be_removed_if_validate, $database->loadResultArray());
			...
			$cf_ids_to_be_removed_if_validate = array_merge($cf_ids_to_be_removed_if_validate, $database->loadResultArray());
			...
			if(is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_small_image')) && is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_medium_image')) && is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_original_image'))) {
				...
				if(
					...
					if(isset($keep_img_ids) && !empty($keep_img_ids)) {
						...
						$tmp_img_ids = $database->loadResultArray();
		...
		elseif( $row->link_published == 1 ) {
			...
			if ( $mtconf->get('notifyadmin_modifylisting') == 1 ) {
				...
				if ( $row->link_approved < 0 ) {
					...
					$subject = JText::sprintf( 'COM_MTREE_MODIFY_LISTING_EMAIL_SUBJECT_WAITING_APPROVAL', $row->link_name);
					$msg .= nl2br(JText::sprintf( 'COM_MTREE_ADMIN_MODIFY_LISTING_MSG_WAITING_APPROVAL', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid"), $row->link_name, $row->link_id, $author->name, $author->username, $author->email, $diff_desc));

				} else {

					$subject = sprintf(JText::_( 'COM_MTREE_MODIFY_LISTING_EMAIL_SUBJECT_APPROVED' ), $row->link_name);
					$msg .= nl2br(JText::sprintf( 'COM_MTREE_ADMIN_MODIFY_LISTING_MSG_APPROVED', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid"), $row->link_name, $row->link_id, $author->name, $author->username, $author->email, $diff_desc));
